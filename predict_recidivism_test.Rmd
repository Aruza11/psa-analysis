...
```{r}
 mdl_best =suppressWarnings(e1071::svm(formula = y~., 
                                        ...
                                        type = 'C-classification',
                                        kernel = 'radial',
                                        gamma = param_svm[1, "gamma"],
                                        epsilon = param_svm[1, "epsilon"],
                                        cost = param_svm[1, "cost"],
                                        # cross = 5,
                                        scale = TRUE, 
                                        probability = TRUE))
  
  X = train2%>% select(-c(y))%>%data.matrix()
  y = train2%>% select(y) %>% unlist()%>% as.numeric()
  
  preds = predict(mdl_best, newdata=X, 
                  probability = TRUE
                  ) 
    # as_data_frame()
    # select(`1`)%>%
...
out_svm = train %>%
          mutate(y = as.factor(recid_use)) %>%
          select(-recid_use) %>%
          fit_svm_auc(param_svm, setup_svm)
