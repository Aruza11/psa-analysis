...
```{r}
  ...
  x1=(data.matrix(train_fold$X$`1`))
  y1=as.numeric((data.matrix(train_fold$y$`1`)))
  fit = glmnet(x=x1, y=y1, alpha=1, family = "binomial",s=best_lambda)
  cv.glmmod <- cv.glmnet(x=x1, y=y1, alpha = 1)
  plot(cv.glmmod)
  best_lambda <- cv.glmmod$lambda.min
  prediction=predict(fit, type = "class", newx = data.matrix(train_fold$X$`2`), s=best_lambda)
  # %>%as.numeric()
