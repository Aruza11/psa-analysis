...
log_path = "C:/Users/binha/Documents/Duke/Cynthia Research/KY-analysis-mytrials/broward/broward models/Baseline Model Results/Minor Offenses/Recidivism/"
```
...
load(paste0(data_path, "expanded_features_minor_offenses.Rdata"))
```
...
features_filt = features_before_on %>%
  ...
  inner_join(features_on, by = c("person_id","screening_date")) %>%
    ...
    inner_join(
        features %>% 
            select(person_id, screening_date, years_since_last_crime,
...
param_xgb <- list(
  ...
  eta = c(0.1, 0.2, 0.3),
  gamma = c(14,15,16,17,18),
  max_depth = c(1,2,3,4, 5),
  min_child_weight = c(14,15,16,17,18),
  subsample = c(1),
...
log_performance(out_xgb, log_path, "xgb")
...
The code below is for lasso and logistic regression. I used Python to tune the two models, as the package the current code is using does not have the tuning choices. Need to change the whole function and package in order to tune. Therefore, temperorily switch those two models into Python. It is expected that there should not be huge differences. But will get back and modify the model.
