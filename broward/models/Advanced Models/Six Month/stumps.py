...
def stump_plots(features, coefs):
    ...
    def stump_visulization(label, sub_features, features, coefs):
        ...
        ## select features
        if ((label == 'current_charge') | (label == 'age_first_offense')):
            ...
            if len(sub_features) == 1:
                ...
                for j in sub_features:
                    ...
                    cutoffs.append(float(j[j.find('=')+1:])) 
    ...
    labels = ['sex', 'current_age','age_first_offense', 'charge', 'jail30', 'prison', 'probation', 'juv_fel_count', 
              'felprop_violarrest', 'murder_arrest', 'felassault_arrest', 'misdemassault_arrest', 'famviol_arrest', 
              'sex_arrest', 'weapons_arrest', 'fail_appear_two_yr', 'fail_appear_two_plus', 'current_violent','current_violent20', 
              'pending_charge', 'prior_conviction_F','prior_conviction_M', 'violent_conviction', 'total_convictions',
              'arrest', 'property', 'traffic', 'drug', 'dui', 'domestic','stalking', 'voyeurism', 'fraud', 'stealing', 'trespass',
              'years_since_last_crime', 'six_month', 'one_year', 'three_year','five_year']
