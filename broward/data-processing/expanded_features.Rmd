...
```{r}
a = charge_df %>% filter(is.na(is_dui))
```
...
new <- outcomes %>% 
  na.exclude()
new %>% select(recid_two_year, recidnot, years)
overall_amt = sum(new$recid_two_year, na.rm = TRUE)
outcomes$recid_two_year[1:10]
nrow(new)
