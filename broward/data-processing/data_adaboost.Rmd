...
not_remove = c("person_id", "recid_use")
names2rm = setdiff(name_features_filt, not_remove)
...
df = features_filt %>%
  ...
  mutate_at(vars(-person_id, -recid_use),
...
```{r}
write.table(train, file=paste0(data_path, "ada_train.csv"),
            sep=",", row.names = F)
write.table(test, file=paste0(data_path, "ada_test.csv"),
            sep=",", row.names = F)
