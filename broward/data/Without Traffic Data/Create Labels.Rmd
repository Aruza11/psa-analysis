...
features_filt = features_before_on %>%
  ...
  inner_join(features_on, by = c("person_id","screening_date")) %>%
    ...
    inner_join(
        features %>% 
            select(person_id, screening_date, six_month, one_year, three_year, five_year), 
        by = c("person_id", "screening_date")) %>% 
...
train = merge(x=train, y=labels, by=c('person_id', 'screening_date')) %>% 
    select(person_id:recid_use, 
           ...
           recid_F,
           recid_violent)%>% 
    mutate(recid_use = as.factor(recid_use),
           recid_drug = as.factor(recid_drug),
           recid_traffic = as.factor(recid_traffic),
...
for (i in labels){
    ...
    subtrain = subtrain[!is.na(subtrain[i]), ]
    
    subtest = test %>% select(person_id:five_year, i)
    subtest = subtest[!is.na(subtest[i]), ]
    
    write.csv(subtrain, file = paste0("train_", i, ".csv"), row.names = F)
    write.csv(subtest, file = paste0("test_", i, ".csv"), row.names = F)
}
