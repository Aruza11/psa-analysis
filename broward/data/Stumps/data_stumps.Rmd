...
```{r}
outcomes = outcomes %>% select(person_id, screening_date, recid, recid_violent)
```
...
features_filt = features_before_on %>%
  ...
  inner_join(outcomes, by = c("person_id","screening_date")) %>%
    inner_join( features %>% select(person_id, screening_date, one_year, three_year, five_year), 
        by = c("person_id", "screening_date")) %>% 
...
```{r}
write.csv(train, file="train_stumps.csv", sep=",", row.names = F)
write.csv(test, file="test_stumps.csv",sep=",", row.names = F)
```
