...
#all_features = all_features[, c(1,2,31,3:6,44,34,7:30,32:33,38:40,35:37,41:43)]
```
...
The reason we use 3 years here is that for those people who are sentenced to jail/prison for 1year or less, we'll use the date when they are released and see if they recidivated or not. We want to make sure that everyone does have the same two-year period to be checked.
...
```{r}
### modified on 10/9/2019
#before_cutoff = before_cutoff0[, c(1,45,2:44)] 
...
## first layer of group: for fta & probation information, since they are based on CaseID level. 
    ...
    features = data %>% 
        ...
        summarise(age_at_current_charge = max(age_at_charge),
                  ...
                  ## added on 10/12/2019
                  #p_incarceration = max(incarceration),
...
## third layer: individual level; sum up all the SeqCaseID information related to one person.
        ...
        summarise(age_at_current_charge = max(age_at_current_charge),
                  ...
                  ## added on 10/12/2019
                  #p_incarceration = sum(p_incarceration),
...
features_before = create_features(data_before) %>% 
    select(PersonID, Race, Gender, screening_date, 
           six_month, 
           one_year, 
           three_year, 
           five_year, 
...
#features_before_on = features_before_on[,c(1:35,42:49,36:41)]
```
...
release_dates = data_on %>% 
    ...
    select(PersonID, release_date, current_SentMonth, 
           current_SuspSentMonth, current_CondDisMonth)
...
after_cutoff = merge(x=after_cutoff0, y=release_dates, by="PersonID", all.y = TRUE) %>% 
    ...
    select(-current_SentMonth, -current_SuspSentMonth, -current_CondDisMonth)
...
```{r}
mean(recid_labels$general_two_year == 1)
mean(recid_labels$general_six_month == 1)
```


```{r}
age_dist = recid_labels %>% 
  select(age_at_current_charge, general_two_year, general_six_month) %>% 
  group_by(age_at_current_charge) %>% 
  summarise(recid_two_year_prob = mean(general_two_year == 1))
...
age_dist1 = pop1 %>% 
  select(age_at_current_charge, general_two_year) %>% 
  group_by(age_at_current_charge) %>% 
  summarise(recid_two_year_prob = mean(general_two_year == 1))
...
#write.csv(test_index, file = "C:/Users/binha/Documents/Duke/Cynthia #Research/KY-analysis-mytrials/KY Recidivism/KY data/kentucky_test_index.csv", row.names = F, #col.names = F)
#
#write.csv(fold, file = "C:/Users/binha/Documents/Duke/Cynthia Research/KY-analysis-mytrials/KY #Recidivism/KY data/fold_index.csv", row.names = F, col.names = F)
```
...
```{r}
test_index = 1:nrow(recid_labels)
