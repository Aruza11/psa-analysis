...
# ProPublica analysis
#f_propub = "../compas-analysis-master/"
f_propub = "../../Desktop/Research_Data/"

# Data we have pulled
#f_our_data = "../../broward-data/"
f_our_data = "../../Desktop/Research_Data/"
```
...
charge_df = charge_df %>%
  ...
  mutate(
    ...
    is_traffic = if_else(substr(charge_degree, 0, 4) =="(TCX)" | charge_degree=="(0)",1,0),
  ...
  #keywords of stalking, harassment, 
    ...
    is_voyeurism = if_else(substr(statute,1,6) %in% c("810.14"),1,0) ,
    ...
    #burglary trespass and voyeurism
...
overall_amt = sum(outcomes$recid)
not_overall_amt = sum(outcomes$recidnot)
drug_amt = sum(outcomes$recid_drug, na.rm = TRUE)

property_amt = sum(outcomes$recid_property)
stalking_amt = sum(outcomes$recid_stalking)
dom_amt = sum(outcomes$recid_domestic)
trespass_amt = sum(outcomes$recid_trespass)
traffic_amt = sum(outcomes$recid_traffic)
voy_amt = sum(outcomes$recid_voyeurism)
fraud_amt = sum(outcomes$recid_fraud)
steal_amt = sum(outcomes$recid_stealing)
dui_amt = sum(outcomes$recid_dui)

voverall_amt = sum(outcomes$recid_violent)
vproperty_amt = sum(outcomes$recid_property_violent)
vstalking_amt = sum(outcomes$recid_stalking_violent)
vdom_amt = sum(outcomes$recid_domestic_violent)
vtrespass_amt = sum(outcomes$recid_trespass_violent)
vtraffic_amt = sum(outcomes$recid_traffic_violent)
vvoy_amt = sum(outcomes$recid_voyeurism_violent)
vfraud_amt = sum(outcomes$recid_fraud_violent)
vsteal_amt = sum(outcomes$recid_stealing_violent)
vdui_amt = sum(outcomes$recid_dui_violent)
vdrug_amt = sum(outcomes$recid_drug_violent, na.rm = TRUE)


crimes <- c("Not","Overall", "Drug", "Property", "Stalking", "Domestic Violence", "Trespass", "Traffic", "Voyeurism", "Fraud", "Stealing", "DUI")
amt <- c(not_overall_amt, overall_amt, drug_amt, property_amt, stalking_amt, dom_amt, trespass_amt, traffic_amt, voy_amt, fraud_amt, steal_amt, dui_amt)
vamt <- c(not_overall_amt, voverall_amt, vdrug_amt, vproperty_amt, vstalking_amt, vdom_amt, vtrespass_amt, vtraffic_amt, vvoy_amt, vfraud_amt, vsteal_amt, vdui_amt)
out_graph = data.frame(Crime = crimes, Frequency = amt, Frequency_Violent = vamt)

out_graph


ggplot(data = out_graph, mapping = aes(x = Crime, y = Frequency, fill = "green")) +
